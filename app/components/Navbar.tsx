...
export default function Navbar() {
    ...
    return (
        <nav className="w-full relative  flex items-center justify-between px-10 py-5 border-b-2 gap-4">
            {/* overflow-auto */}
            <div>
            ...
            <div className="flex flex-row gap-4 lg:gap-8">
                <div className="hidden sm:flex sm:items-center sm:justify-between lg:gap-8">
                    {routes.map((route) => 
                        <Link 
                            href={route.href} 
                            className={(`px-3 py-2 font-semibold text-sm md:text-base lg:text-lg ${route.active && 'text-blue-500'}`)}>
                            {route.label}
                        </Link>    
                    )}
                </div>
                ...
                <div className="flex items-center justify-between">
                    <ModeToggle />
                    <SearchButton />
                    <Button variant="ghost" size="sm" onClick={() => {setOpen(true)}} className="sm:hidden">
                        <Menu />
                    </Button>
                </div>
            </div>
